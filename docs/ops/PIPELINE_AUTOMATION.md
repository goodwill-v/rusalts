...
## Источники парсинга (официальные ссылки)

Список источников для ежедневного мониторинга хранится в `parser/sources.json`.
Если в логах парсера появляются `404` (страница не найдена) — обычно нужно обновить URL источника.

Для удобства редактирования в “табличном виде” используйте экспорт в CSV (открывается в Excel):
